...
public:
    ...
    bool lbd_block_completed();
    std::vector<std::string> get_options_to_complete_lbd_block();
    std::string to_string();

    Cube3& operator = (const Cube3& rhs) {
        this->_front_color = rhs._front_color;
        this->_back_color = rhs._back_color;
        this->_left_color = rhs._left_color;
        this->_right_color = rhs._right_color;
        this->_up_color = rhs._up_color;
        this->_down_color = rhs._down_color;
        for (int i = 0; i < 10; ++i) {
            this->_face_front[i] = rhs._face_front[i];
            this->_face_back[i] = rhs._face_back[i];
            this->_face_left[i] = rhs._face_left[i];
            this->_face_right[i] = rhs._face_right[i];
            this->_face_up[i] = rhs._face_up[i];
            this->_face_down[i] = rhs._face_down[i];
        }
        return *this;
    }

    bool operator == (Cube3& rhs) {
...
std::string Cube3::to_string() {
    std::string s;
    char color_list[] = "YRWBGO";
    char * pface[] = {_face_front, _face_back, _face_up, _face_down, _face_left, _face_right};
    char * parray[6] = {NULL};
    for (int i = 0; i < 6; ++i) {
        for (int j = 0; j < 6; ++j) {
            if (pface[j][8] == color_list[i]) {
                parray[i] = pface[j];
            }
        }
    }
    int to_row_col[] = {0, 1, 2, 7, 8, 3, 6, 5, 4};
    for(int i = 0; i < 6; ++i) {
        for (int j = 0; j < 9; ++j) {
            s += parray[i][to_row_col[j]];
        }        
    }
    return s;
}
...
void Cube3::restore_directly() {
    for (int i = 0; i < 9 ; ++i) { // 一个面有8个块和1个中心块
    ...
    _face_front[9] = _face_back[9] = _face_left[9] = _face_right[9] = _face_up[9] = _face_down[9] = '\0';
}
...
std::vector<std::string> Cube3::get_face_9_color(char op) {
    ...
    char tmp[9] = {pface[0], pface[1], pface[2], pface[7],pface[8], pface[3], pface[6], pface[5], pface[4]};
...
void Cube3::disturb_cube(const std::vector<std::string>& disturb_list) {
    this->restore_directly();
...
bool Cube3::lbd_block_completed() {
    for (int i = 5; i <= 7; ++i) {
        if (_face_left[i] != _left_color) {
            return false;
        }
    }
    for (int i = 3; i <= 5; ++i) {
        if (_face_back[i] != _back_color) {
            return false;
        }
    }
    for (int i = 5; i <= 7; ++i) {
        if (_face_down[i] != _down_color) {
            return false;
        }
    }
    return true;
...
std::vector<std::string> Cube3::get_options_to_complete_lbd_block() {
    std::vector<std::string> options;
    //std::map<Cube3, std::string> mp;
    if (this->lbd_block_completed()) {
        return options;
    }
    std::cout<<"当前LBD未被还原\n";
    std::map<std::string, int> vis;
    std::queue<std::pair<Cube3, std::string> > Q;
    Q.push(std::make_pair(*this, std::string("")));
    char op_list[] = "lrudfb";
    std::cout<<"初始化完毕\n";
    int cntt = 0;
    while (!Q.empty()) {
        Cube3 tp = Q.front().first;
        std::string tp_str = Q.front().second;
        Q.pop();
        ++cntt;
        if(cntt%10000==0) {
            std::cout<<tp_str<<"\n";
        ...
        for (int i = 0; i < 6; ++i) {
            //std::cout<<"i = "<<i<<"\n";
            Cube3 to = tp;
            std::string op_rotate = "";
            op_rotate += (char)toupper(op_list[i]);

            to.rotate_op(op_rotate);
            
            std::string new_str = tp_str;
            new_str += op_rotate;
            //std::cout<<new_str<<std::endl;
            if (vis.find(to.to_string()) != vis.end()) {
                continue;
            }
            if (to.lbd_block_completed()) {
                options = Cube3::prase_string_to_rotate_op(new_str);
                break;
            }
            Q.push(std::make_pair(to, new_str));
            vis[to.to_string()] = 1;
        }
    }
    // 无解和已经好了都是options， 不过默认有解，除非魔方被拆过，本身无法还原
    return options;
}
...
//        std::vector<std::string> op_list = Cube3::get_a_random_disturb();
        std::string ddst = "B2 D2 L2 B' D2 B D2 L2 F R2 U' B' U' R' D2 B F2 L2 F"; 
        std::vector<std::string> op_list = Cube3::prase_string_to_rotate_op(ddst);
    ...
    std::cout<<"还原LBD块的一组可行操作：\n";
    std::vector<std::string> v_complete_lbd = p->get_options_to_complete_lbd_block();
    int len = v_complete_lbd.size();
    for (int i = 0; i < len; ++i) {
        std::cout<< v_complete_lbd[i]<<" ";
        p->rotate_op(v_complete_lbd[i]);
    }
    std::cout<<std::endl;
    std::cout<<"还原LBD后：\n";
    p->show_all_face_color_graphically();
